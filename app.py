...
def register_trader():
    ...
    # Generate trader_name from display_name
    trader_name = display_name.lower().replace(' ', '_')
    ...
    try:
        db.execute(
            "INSERT INTO traders (trader_name, display_name, firm, pin, status) VALUES (?, ?, ?, ?, 'ACTIVE')",
            (trader_name, display_name, firm, pin)
        )
        if pin_row:
